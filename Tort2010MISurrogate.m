function [ShuffleMIs] = Tort2010MISurrogate(lfPhase, hfAmp, numShuffles, binSizes, numPhaseBins) 
...
endDiff = max(vector) - inds(i,2);
while endDiff >= minLength && inds(end,2) < max(vector)
    i=i+1;
...
if inds(end,2) < max(vector) %this probably always happens
    switch max(vector) - inds(end,1) <= maxLength
        case 1 %if we can just add it to the end and still be good
            inds(end,2) = max(vector);
        case 0    
            deficit = max(vector) - inds(end,2);
            ...
            switch deficit > minLength/2
                case 1 %make a new bin, steal some from the last one
                    ...
                    inds(size(inds,1)+1,:) = [inds(size(inds,1),2)+1 max(vector)];
...
for shuffledInd=1:size(inds,1)
    hfAmpShuffled = [hfAmpShuffled hfAmp(inds(shuffledInds(shuffledInd),1):inds(shuffledInds(shuffledInd),2))];
end

% 4. Get an MI on this
[MI, ~, ~, ~] = Tort2010MI(lfPhase, hfAmpShuffled, numPhaseBins);
